 ...
 */
(function () {
  const API = window.ARQON_API_BASE || '/PROJETO-ARQUON/index.php/api';
  const img = window.arqonImageUrl || ((p) => p || 'https://placehold.co/600x800/0a0107/dcb23c?text=ARQON');
  ...
  async function getProdutos() {
    ...
    try {
      ...
      CACHE_PRODUTOS = data.produtos || data.data || [];
    } catch (e) {
  ...
  async function initNewDrops() {
    ...
    if (!produtos.length) { track.innerHTML = ''; return; }
