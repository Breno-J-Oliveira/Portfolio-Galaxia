...
const FEM_API_BASE_URL = '/PROJETO-ARQUON/index.php/api';
...
async function carregarAcervoFeminino() {
    ...
    try {
        ...
        // FILTRO ESTRITO: Captura apenas o universo feminino (MUDADO/CORRIGIDO)
        femCacheProdutos = todosProdutos.filter(p => {
            const cat = (p.categoria || '').toLowerCase();
            const nome = (p.nome || '').toLowerCase();
            return cat.includes('feminin') || 
...
function aplicarDiretrizesTaticas() {
    ...
    // 2. Ordenação Tática
    if (femFiltrosAtivos.ordenacao === 'price-asc') {
        produtosProcessados.sort((a, b) => getValorReal(a) - getValorReal(b));
    } else if (femFiltrosAtivos.ordenacao === 'price-desc') {
        produtosProcessados.sort((a, b) => getValorReal(b) - getValorReal(a));
    } else if (femFiltrosAtivos.ordenacao === 'rarity') {
        produtosProcessados.sort((a, b) => getValorReal(b) - getValorReal(a));
    }
    ...
    produtos.forEach(p => {
        ...
        const cardHTML = `
            <div class="arq-card">
                ...
                <div class="card-info">
                    <p class="card-brand">AQ-F-${(p.id || '0').toString().padStart(4, '0')}</p>
...
function configurarSensoresDeInterface() {
    const categoryOptions = document.querySelectorAll('#fem-filter-category li, #filter-category li');
    categoryOptions.forEach(option => {
        option.addEventListener('click', (e) => {
            categoryOptions.forEach(opt => opt.classList.remove('active'));
            e.currentTarget.classList.add('active');
            femFiltrosAtivos.categoria = e.currentTarget.getAttribute('data-value');
            aplicarDiretrizesTaticas();
        });
    });

    const selectOrdenacao = document.getElementById('fem-filter-order') || document.querySelector('.m-cyber-select');
    if (selectOrdenacao) {
        selectOrdenacao.addEventListener('change', (e) => {
            femFiltrosAtivos.ordenacao = e.target.value;
            aplicarDiretrizesTaticas();
        });
    }

    const btnClear = document.getElementById('fem-btn-clear') || document.getElementById('btn-clear-filters');
    if (btnClear) {
        btnClear.addEventListener('click', () => {
            femFiltrosAtivos.categoria = 'all';
            femFiltrosAtivos.ordenacao = 'rarity';
...
function getValorReal(produto) {
    if (!produto) return 0;
    if (produto.preco_custodia) return parseFloat(produto.preco_custodia);
    if (produto.preco) return parseFloat(produto.preco);
    return 0;
}
...
function renderizarMascSkeletons(grid, qtd) {
    ...
    for (let i = 0; i < qtd; i++) {
        skeletons += `
            <div class=\"arq-card skeleton-box\">
