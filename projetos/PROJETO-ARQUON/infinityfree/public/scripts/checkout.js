...
console.log('[ARQON CHECKOUT] Script carregado. Inicializando...');

document.addEventListener('DOMContentLoaded', () => {
    console.log('[ARQON CHECKOUT] DOMContentLoaded disparado.');
    const token = localStorage.getItem('arqon_token');
    
    // Headers para requisições autenticadas (se houver token)
    ...
    async function carregarCarrinho() {
        console.log('[ARQON CHECKOUT] Iniciando carregarCarrinho...');
        console.log('[ARQON CHECKOUT] ARQON_CONFIG.apiBaseUrl:', ARQON_CONFIG.apiBaseUrl);

        // Primeiro tenta do localStorage (sempre funciona, mesmo sem login)
        const cartData = localStorage.getItem('arqon_cart');
        console.log('[ARQON CHECKOUT] localStorage.getItem("arqon_cart"):', cartData);
        carrinhoItens = JSON.parse(cartData) || [];
        console.log('[ARQON CHECKOUT] Carrinho do localStorage:', carrinhoItens);
        
        if (carrinhoItens.length > 0) {
            console.log('[ARQON CHECKOUT] Itens encontrados no localStorage. Renderizando...');
            renderizarCarrinho(carrinhoItens);
            calcularTotais(carrinhoItens);
            return;
        }
        
        // Se localStorage vazio e estiver logado, tenta da API
        if (!isVisitante) {
            console.log('[ARQON CHECKOUT] localStorage vazio e usuário logado. Tentando carregar da API...');
            try {
                const response = await fetch(`${ARQON_CONFIG.apiBaseUrl}/carrinho`, { headers: headersAutenticados });
                const result = await response.json();

                if (result.status === 'success' && result.data && result.data.length > 0) {
                    console.log('[ARQON CHECKOUT] Itens carregados da API:', result.data);
                    carrinhoItens = result.data;
                    renderizarCarrinho(carrinhoItens);
                    calcularTotais(carrinhoItens);
                } else {
                    console.log('[ARQON CHECKOUT] API retornou vazio ou erro:', result);
                    renderizarCarrinho([]);
                    calcularTotais([]);
                }
            } catch (error) {
                console.error('[ARQON CHECKOUT] Erro ao carregar carrinho da API:', error);
                renderizarCarrinho([]);
                calcularTotais([]);
            }
        } else {
            console.log('[ARQON CHECKOUT] Usuário visitante e localStorage vazio.');
            renderizarCarrinho([]);
            calcularTotais([]);
        }
    }
        ...
        container.innerHTML = itens.map(item => `
            <div class="checkout-item">
                <div class="item-visual">
                    <img src="${resolveImg(item.foto_url || item.imagem)}" alt="${item.nome || 'Artefato'}" loading="lazy" onerror="this.onerror=null;this.src='https://placehold.co/400x500/0a0107/dcb23c?text=ARQON';">
                </div>
