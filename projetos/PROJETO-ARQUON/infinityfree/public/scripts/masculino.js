...
async function carregarAcervoMasculino() {
    ...
    try {
        ...
        // FILTRO ESTRITO: Captura apenas o universo masculino (CORRIGIDO)
        mascCacheProdutos = todosProdutos.filter(p => {
            const gen = (p.genero || 'Unissex').toLowerCase();
            return gen.includes('masculin') || gen.includes('unissex');
        });
...
function aplicarDiretrizesTaticas() {
    ...
    if (mascFiltrosAtivos.categoria !== 'all') {
        const filtro = mascFiltrosAtivos.categoria.toLowerCase();
    ...
    if (mascFiltrosAtivos.estilo !== 'all') {
        const filtro = mascFiltrosAtivos.estilo.toLowerCase();
        produtosProcessados = produtosProcessados.filter(p => {
    ...
    if (mascFiltrosAtivos.marca !== 'all') {
        const filtro = mascFiltrosAtivos.marca.toLowerCase();
        produtosProcessados = produtosProcessados.filter(p => {
    ...
    if (mascFiltrosAtivos.status && mascFiltrosAtivos.status !== 'all') {
        const s = mascFiltrosAtivos.status;
    ...
    if (mascFiltrosAtivos.precoMax) {
        produtosProcessados = produtosProcessados.filter(p =>
            getValorReal(p) <= mascFiltrosAtivos.precoMax
        );
    }

    // 6. Ordenação
    switch (mascFiltrosAtivos.ordenacao) {
        case 'menor-preco':
...
function configurarSensoresDeInterface() {
    // Categoria
    document.querySelectorAll('#masc-filter-category li').forEach(li => {
        li.addEventListener('click', () => {
            document.querySelectorAll('#masc-filter-category li').forEach(x => x.classList.remove('active'));
            li.classList.add('active');
            mascFiltrosAtivos.categoria = li.dataset.value;
            aplicarDiretrizesTaticas();
        });
    });

    // Estilo
    document.querySelectorAll('#masc-filter-estilo li').forEach(li => {
        li.addEventListener('click', () => {
            document.querySelectorAll('#masc-filter-estilo li').forEach(x => x.classList.remove('active'));
            li.classList.add('active');
            mascFiltrosAtivos.estilo = li.dataset.value;
            aplicarDiretrizesTaticas();
        });
    });

    // Marca
    document.querySelectorAll('#masc-filter-marca li').forEach(li => {
        li.addEventListener('click', () => {
            document.querySelectorAll('#masc-filter-marca li').forEach(x => x.classList.remove('active'));
            li.classList.add('active');
            mascFiltrosAtivos.marca = li.dataset.value;
            aplicarDiretrizesTaticas();
        });
    });

    // Status de disponibilidade
    document.querySelectorAll('#masc-filter-status li').forEach(li => {
        li.addEventListener('click', () => {
            document.querySelectorAll('#masc-filter-status li').forEach(x => x.classList.remove('active'));
            li.classList.add('active');
            mascFiltrosAtivos.status = li.dataset.value;
            aplicarDiretrizesTaticas();
        });
    });

    // Ordenação
    document.querySelectorAll('#masc-filter-order li').forEach(li => {
        li.addEventListener('click', () => {
            document.querySelectorAll('#masc-filter-order li').forEach(x => x.classList.remove('active'));
            li.classList.add('active');
            mascFiltrosAtivos.ordenacao = li.dataset.value;
            aplicarDiretrizesTaticas();
        });
    });

    // Slider de preço
    const slider = document.getElementById('masc-price-slider');
    const display = document.getElementById('masc-price-display');
    if (slider && display) {
        slider.addEventListener('input', () => {
            mascFiltrosAtivos.precoMax = parseInt(slider.value);
    ...
    // Limpar
    const btnClear = document.getElementById('masc-btn-clear');
    if (btnClear) {
        btnClear.addEventListener('click', () => {
            mascFiltrosAtivos = { categoria: 'all', estilo: 'all', marca: 'all', status: 'all', ordenacao: 'recentes', precoMax: 5000 };
            document.querySelectorAll('.masc-filter-options li').forEach(li => {
...
function alternarWishlistCard(produtoId) {
    ...
    fetch(`${MASC_API_BASE_URL}/wishlist/verificar?id_produto=${produtoId}`, {
    ...
    .then(result => {
        if (result.status === 'success' && result.na_wishlist) {
            // Remove
            return fetch(`${MASC_API_BASE_URL}/wishlist?id_produto=${produtoId}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${token}` }
            });
        } else {
            // Adiciona
            return fetch(`${MASC_API_BASE_URL}/wishlist`, {
                method: 'POST',
