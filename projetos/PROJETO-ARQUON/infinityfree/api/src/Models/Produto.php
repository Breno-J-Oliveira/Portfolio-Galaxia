...
class Produto {
    ...
    public function buscarDetalhesPorId(int $id): ?array {
        try {
            ...
            if (!$resultado) {
                error_log("[ARQON MODEL] Produto ID $id não encontrado");
                return null;
            }

            error_log("[ARQON MODEL] Produto ID $id encontrado: " . $resultado['nome']);

            // Converte foto_principal_url para foto_url para compatibilidade com frontend
            if (isset($resultado['foto_principal_url']) && $resultado['foto_principal_url']) {
                $resultado['foto_url'] = $resultado['foto_principal_url'];
            } else {
                // Fallback para placeholder se não tiver foto
                $resultado['foto_url'] = 'https://placehold.co/400x500/0a0107/dcb23c?text=ARQON';
            }

            // Tenta buscar galeria, mas não falha se não conseguir
            try {
                $sqlMidias = "SELECT url_midia FROM produto_midias WHERE id_produto = :id AND tipo_midia = 'imagem' ORDER BY ordem ASC";
                ...
                $resultado['midias_extras'] = $midias ? $midias : [];
            } catch (Exception $e) {
                error_log("[ARQON MODEL] Erro ao buscar galeria: " . $e->getMessage());
                $resultado['galeria'] = [];
                $resultado['midias_extras'] = [];
            }

            return $resultado;

        } catch (Exception $e) {
            error_log("[ARQON MODEL] Erro em buscarDetalhesPorId: " . $e->getMessage());
            throw $e; 
        }
    }
    ...
    public function buscarPorTexto(string $query): array {
        ...
        foreach ($result as &$row) {
            if (isset($row['foto_principal_url']) && $row['foto_principal_url']) {
                $row['foto_url'] = $row['foto_principal_url'];
            } else {
                // Fallback para placeholder se não tiver foto
                $row['foto_url'] = 'https://placehold.co/400x500/0a0107/dcb23c?text=ARQON';
            }
        }
        return $result;
    }
    ...
    public function buscarCatalogoInteligente(
        ...
        $sql = "SELECT p.id, p.nome, p.descricao, p.valor_diaria, p.valor_mercado, 
                       p.foto_principal_url, p.composicao, p.status_venda,
                       m.nome AS marca, 
                       c.nome AS categoria,
                       e.nome AS estilo
                FROM produtos p 
    ...
    public function inserir(array $dados): int|bool {
        ...
        try {
            ...
            $sucesso = $stmt->execute([
                ...
                ':foto_url'      => $dados['foto_url'] ?? $dados['foto_principal_url'] ?? null,
